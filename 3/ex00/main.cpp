...
int main()
{
	...
	std::cout << "Watch out! a wild Clap Trap!" << std::endl;
	...
	while (1)
	{
		...
		if (prompt != "INFO")
		{
			if (ChilChunck.get_EP() <= 0)
				std::cout << ChilChunck.get_name() << " has no energy left!" << std::endl;
			else if (ChilChunck.get_HP() < 5)
				ChilChunck.beRepaired(3);
			else
			{
				ChilChunck.attack(hero_name);
				if (hero_def == true)
				{
					std::cout << "The Hero was defending! it took a third of the damage" << std::endl;
					hero_HP -= ChilChunck.get_AD() / 3;
				}
				else 
					hero_HP -= ChilChunck.get_AD();	
				hero_def = false;
			}
		}
		if (ChilChunck.get_HP() <= 0)
		{
			std::cout << ChilChunck.get_name() << " WAS DEFEATED!!!" << std::endl;
			std::cout << "YOU WIN!!!" << std::endl;
			return (0);
		}
		if (hero_HP <= 0)
		{
			std::cout << ChilChunck.get_name() << " DEALED FATAL DAMAGE" << std::endl;
